...
class opencv:
    ...
    def camera(self):
        ...
        for i in self.face_arr:
            ...
            im.save('./image/' + input_food + '/' + str(datetime.now()) + '.png')
    ...
    def model(self, update):
        x_train = []
        num_classes = 4
        image_cnt = 0

        for i in os.listdir('./image'):
            image_cnt += len(os.listdir('./image/' + i))

        y_label = np.ones((image_cnt, num_classes))
        for j in os.listdir('./image'):
            self.face_arr = []

            for i in os.listdir('./image/' + j):
                self.face_arr.append((cv2.imread('./image/' + j + '/' + i)))
                image_cnt += 1
        ...
        if update == 0:
            model = Sequential()
            model.add(Conv2D(64, (8, 8), input_shape=input_shape, padding='same', activation='relu', kernel_constraint=maxnorm(3)))
